...
N = 101;
h = (7 - (-3)) / (N - 1);
x = zeros(N, 1); % Preallocate x and y
y = zeros(N, 1);
...
% appropriate comments.
N = 5000001;
h = (3 - (-2)) / (N - 1); % Distance between points
x = zeros(N, 1); % Preallocate x
...
% Clear y and populate with for-loop
clear y;
...
% Clear y and populate with vectorization
clear y;
tic;
y = atan(x);
toc;
...
% Initialize t as independent variable
t = linspace(1, 5);

% Initialize particular solutions
y_1 = (t - exp(1) - 1)./exp(t.^2);
y_2 = (t - 1)./exp(t.^2)
...
% Init x with 100 grid points
x = linspace(-2, 2);
y = zeros(size(x)); % Preallocate y

% Use fzero in loop
for j = 1:length(x)
    y(j) = fzero(@(y) f(x(j), y), 0);
end
...
% General solution implicit form -> x^3 - sin(x) -ln|y| + C = 0
% Explicitly -> y = e^(x^3 - sin(x) + C)
% For y(0) = 1 -> x^3 - sin(x) -ln|y| = 0

% Define function
f = @(x, y) x.^3 - sin(x) - log(abs(y));
x = linspace(-1.5, 1.25); % Populate x
y = zeros(size(x)); % Preallocate y

% Populate y using fzero
for i = 1:length(x)
...
% As x -> inf, y -> inf when y > 0. As x -> inf, y -> 0 when y <= 0
% The function is always even, making it symmetrical about y. General
% solution y = -2 / (x^2 + C) is an even function. This will remain
% an even function for all values of the constant C, i.e. f(-x) = f(x) at
% all times since (-x)^2 = x^2.
...
%�y(0) = -2, y -> -1 as x -> inf. y -> -inf as x -> -inf.
% y(0) = 0, y -> 1 as x -> inf. y -> -1 as x -> -inf.
% y(0) = 3, y -> inf as x -> inf. y -> 2 as x -> -inf
% y(0) = 1.5, y -> 1 as x -> inf. y -> 2 as x -> -inf

% y(0) < -1, y -> -1 as x -> inf, y-> -inf as x -> -inf 
% 1 > y(0) > -1, y -> 1 as x -> inf, y-> -1 as x -> -inf
% 2 > y(0) > 1, y -> 1 as x -> inf, y-> 2 as x -> -inf
% y(0) > 2, y -> inf as x -> inf, y-> 2 as x -> -inf
