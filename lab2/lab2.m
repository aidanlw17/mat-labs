...
%% Student Information
%
% Student Name: Aidan Lawford-Wickham
%
% Student Number: 1005089226
%
...
% Define the function
f = @(t, y) y*tan(t) + sin(t);
% Provide initial conditions and final time
t_i = 0;
t_f = pi;
y_i = -1/2;

% Solve ODE
solution = ode45(f, [t_i, t_f], y_i);
...
% Plot solutions
plot(tt, yy, solution.x, solution.y, 'x', 'MarkerSize', 10, 'LineWidth', 2);
xlabel('t')
ylabel('y')
legend('Exact', 'Numerical', 'Location', 'Best')
...
% Create range of grid points and compute solution
tinterp = linspace(2, 3, 10)
deval(solution, tinterp)
...
% Compute the exact solution
yexact = (-0.5).*cos(solution.x);

% Compute pointwise error
error = abs(yexact - solution.y);

disp(error)

fprintf('Maximum error: %g \n', max(error));

% Visualize error on linear-log
semilogy(solution.x, error, 'LineWidth', 2);
xlabel('t');
ylabel('error');

% Largest error is 1.807e-05, which occurs at t = 1.559 or pi/2.
% Largest error occurs here since tan(pi/2) is undefined, and the
% function y = tan(t) approaches infinity as t approaches pi/2.
% This makes the ratio between y and t step very large.
...
% Define ODE inline and init conditions
f = @(t, y) 1/(y.^2);
t_0 = 1;
y_0 = 1;
...
% Define init conditions and integral limit
t_0 = 0;
y_0 = -1;
t_limit = 10;

% Compute numerical solution
solution = ode45(f, [t_0, t_limit], y_0);
disp(solution);

% Plot solution
plot(solution.x, solution.y, 'x', 'MarkerSize', 10, 'LineWidth', 2);
xlabel('t');
ylabel('y');
...
% Define the ODE inline
f = @(t, y) y.^3 - t.^2;

% Initial conditions and integral limit
t_0 = 0;
y_0 = 1;
t_limit = 1;
...
% The given error message: "Warning: Failure at t=5.066046e-01.  Unable to
% meet integration tolerances without reducing the step size below the 
% smallest value allowed (1.776357e-15) at time t."

% This error message occurs for the same reason we saw a peak in the error
% in exercise 4. The solution approaches an asymptote at this value of t,
% taking the step size out of the tolerance range and requiring it to be
% decreased until eventually the smallest step size is reached.
...
% Define and simplify function
f = sin(x).*cos(x);
s = simplify(f);

% Plot simplified function
ezplot(s);

% Compute values for the function
value_1 = subs(f, x, pi/4);
disp(value_1);
value_2 = eval(subs(f, x, 1));
disp(value_2);
...
% Get approximations over a loop
N=5;
for i = 1:N
    % Prepare integration
    func=subs(f,y,phi);    
    ...
    phi=cat(2,phi,[newphi]);       
end

phi(N+1)

% Plot Picard approximation
picard=ezplot(phi(N+1),[0,5]);
